...
public class BigDecimalUtils {
	...
	//----------------------------------------------------------------------------
	/**
	 * Este método retorna o valor de value arredondado para
	...
	/**
	 * Este método retorna o valor de value truncado para
	 * dec digitos decimais no formato String
	 * @param value
	 * @param dec
	 * @return
	...
	public static BigDecimal bcdToBigDecimal(byte[] b, int scale){
		...
		if (scale > 0) {
			if (scale > hexval.length()) {
				// completa com zeros antes da posi��o de inser��o do ponto decimal
				int count = scale - hexval.length();

			}
				// insere um ponto decimal na posi��o indicada
